...
data_allyears <- c(1:length(sheetnames))%>%
  map_dfr(function(current_sheet_nr){# current_sheet_nr <- 15
    ...
    if(sheetnames[current_sheet_nr] %in% c("y2008")){
      test <- test %>%
        rename(`Unit/\nSector` = `Unit\nSector`)}
    else if(sheetnames[current_sheet_nr] %in% c("y2016")){
      test <- test %>%
        rename(temp = Country)%>%
        rename(Country = "Name of Beneficiary")%>%
        rename("Name of Beneficiary" = temp)
    }
    else{
          test <- test
        }
  ...
  ##
  ## check manually all the NAs here if they are correct
  ##
  filter(!is.na(`eur`))%>%
...
ngo_grants <- data_allyears %>% filter(country == "NGOs for reintegration services")%>%pull(grant_no)

data_allyears_clean <- data_allyears %>%
  mutate(country = if_else(country %in% names(replace_names), str_replace_all(country, replace_names), country),
         country = 
      case_when(
        country %in% valid_country_names$country_name_clean ~ country,
        grepl("Macedonia", country) ~ "North Macedonia",
        country == "NGOs for reintegration services" ~ str_split_i(beneficiary, " - ", 1),
        T ~ country
        ),
    beneficiary = case_when(
      ...
      country == "France" & grepl("ecurite|écurité", beneficiary) & grepl("ublique", beneficiary) ~ "Direction nationale de la Sécurité publique",
      ...
      country == "Iceland" & grepl("National", beneficiary) & grepl("Commissioner", beneficiary) & grepl("Police", beneficiary) ~ "Office of the National Commissioner of the Icelandic Police",
      country == "Iceland" & grepl("flugvelli", beneficiary) ~ "Police department at Keflavík Airport",
      grepl("European Technology and Training Centre", beneficiary) ~ "European Technology and Training Centre",
      ...
      country == "Latvia" & grepl("Office of Citizenship and Migration Affairs", beneficiary) ~ "Office of Citizenship and Migration Affairs",
      ...
      country == "Luxembourg" & grepl("Home Affairs", beneficiary) & grepl("Immigration", beneficiary) ~ "General Department of Immigration - Ministry for Home Affairs",
      ...
      country == "North Macedonia" & grepl("Public Security Bureau", beneficiary) ~ "Public Security Bureau, Ministry of Internal Affairs",
      country == "North Macedonia" & grepl("Ministry", beneficiary) & grepl("Interior|Internal", beneficiary) ~ "Ministry of Internal Affairs Macedonia",
      ...
      country == "Netherlands" & grepl("The National Police of the Netherlands|Netherlands Police", beneficiary) ~ "National Police",
      ...
      country == "Portugal" & grepl("Marinha Portuguesa|Navy", beneficiary) ~ "Portuguese Navy",
      country == "Portugal" & grepl("Estado-Maior da ForÃ§a AÃ©|Air Force|Air Frorce", beneficiary) ~ "Portuguese Air Force",
      ...
      country == "Portugal" & grepl("Sistema de Segurança Interna", beneficiary) & grepl("Homeland Security System", beneficiary) ~ "Sistema de Segurança Interna - Homeland Security System",
      ...
      country == "Portugal" & grepl("Portugese", beneficiary) ~ str_replace_all(beneficiary, "Portugese", "Portuguese"),
      country == "Romania" & grepl("General Inspectorate for Immigration|General Inspectore for Immigration|Romanian Office for Immigration", beneficiary) ~ "General Inspectorate for Immigration",
      ...
      country == "Slovakia" & grepl("Ministry of Interior of the Slovak Republic|Slovakia - Ministry of Interior", beneficiary) ~ "Ministry of Interior",
      ...
      country == "Sweden" & grepl("Swedish Police Authority|Swedish Policy Authority", beneficiary) ~ "Swedish Police Authority",
      ...
      T ~ str_remove_all(beneficiary, "Austria - |Netherlands - |Netherlands-|Iceland - |Hungary - |Germany - |Croatia - |Lithuania - |Luxembourg - |Cyprus - |Moldova - |Iceland- |Bosnia and Hercegovina - |France - |Latvia - |Denmark- |Egypt - |Estonia - |Italy - |Sweden - |Pakistan - |Portugal- |Belgium - |Portugal - ")
    ),
    unit_sector = 
      case_when(
        grant_no == "2009/376a/" ~ "SBS",
        unit_sector == "lBS" ~ "LBS",
        unit_sector %in% c("ECRet","ECRET","Ecret") ~ "ECRET",
        T ~ unit_sector
      ),
    beneficiary = case_when(
      beneficiary == "Ministry of Justice" ~ glue("Ministry of Justice {country}"),
      beneficiary == "Ministry of Interior" ~ glue("Ministry of Interior {country}"),
      grepl("Non-governmental organisation|Non-profit association|Non-profit Organisation|Caritas International|Life Makers|European Technology and Training|Women Empowerment Literacy", beneficiary) | grant_no %in% ngo_grants ~ glue("{ str_remove_all(beneficiary, ', Non-governmental organisation|, Non-profit association')} (NGO)"),
      T ~ beneficiary
      ),
